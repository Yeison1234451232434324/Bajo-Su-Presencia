...
document.addEventListener('DOMContentLoaded', () => {
  ...
  async function renderEventosPublicados() {
    ...
    function renderCard(ev) {
      const chipsRecursos = ev.recursos && ev.recursos.length > 0
        ? ev.recursos.map(r => `<span class="ev-chip-recurso"><i class="bx bx-package"></i> ${r.recursoNombre} × ${r.cantidad}</span>`).join('')
        ...
        <div class="ev-publicado-card" id="ev-card-${ev.id}">
          <div class="ev-pub-header">
            <div class="ev-pub-header-info">
              <div class="ev-pub-titulo-row"><h4 class="ev-pub-titulo">${ev.titulo}</h4>${badgeEstado(ev.estado)}</div>
              <p class="ev-pub-meta">
          ...
          <div class="ev-pub-recursos">
            ...
            <div class="ev-chips-wrap">${ev.especialistas.map(e => `<span class="ev-chip-recurso"><i class="bx bx-user"></i> ${e.nombre} — ${e.especialidad}</span>`).join('')}</div>
          </div>` : ''}
  ...
  window.verEvento = async function(id) {
    ...
    if (!ev) return;
    document.getElementById('modal-ver-titulo').innerHTML = `${ev.titulo} ${badgeEstado(ev.estado)}`;
    const chipsRecursos = ev.recursos && ev.recursos.length > 0
