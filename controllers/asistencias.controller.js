...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  function filaAsistencia(a) {
    const contacto = [a.email, a.telefono].filter(Boolean).join('&nbsp;·&nbsp;');
    const badgeAsistencia = a.asistio
      ? `<span class="asist-badge asist-badge--asistio"><i class="bx bx-qr-scan"></i> Asistió por QR${a.fechaAsistencia ? ` · ${a.fechaAsistencia}` : ''}</span>`
      : `<span class="asist-badge asist-badge--falta"><i class="bx bx-x-circle"></i> No asistió</span>`;
    const califHint = tieneCalificacion(a)
      ? `<span class="asist-row-calif" title="Ya calificó"><i class="bx bxs-star"></i> ${a.calificacion}/5</span>`
      : '';
    ...
    return `
      <div class="asist-row ${a.asistio ? 'asist-row--asistio' : 'asist-row--falta'}">
        ...
        <div class="asist-row-info">
          <p class="asist-row-nombre">${a.nombre} ${califHint}</p>
          ${contacto ? `<p class="asist-row-sub">${contacto}</p>` : ''}
        </div>
        <div class="asist-row-estados">
          <span class="asist-badge asist-badge--inscrito"><i class="bx bx-user-plus"></i> Inscrito${a.fechaInscripcion ? ` · ${a.fechaInscripcion}` : ''}</span>
          ${badgeAsistencia}
        </div>
      </div>`;
  }
